% computes the performance specific to sides in each individual binmix days, and plots them across days onto the same plot. 

%============================
function [ACCR, ACCR1, ACCR0] = scoreanalysis(csvpath, n)
cd (csvpath)
...
% create figure format axes and plot markings.
figure (n)
...
% go through .csv files to analyse each day
for i=3:length(dir)
...
% plot performance for that day as a data point with size indicating total number of responses
...
% plot also the overall and side specific performance across days in a 1-D time series.
subplot(3, 2, 5:6)
hold on
