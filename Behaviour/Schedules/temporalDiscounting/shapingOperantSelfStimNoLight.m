...
% * Arbitrary numeric arrays and strings may be included for storage in the header.
function scheduleData = shapingOperantSelfStimNoLight
...
scheduleData.minRewardDelay=0.1;
scheduleData.maxVarRewardDelay=0.1;
scheduleData.timeToShoot=0;
%scheduleData.timeToShoot='reward';
scheduleData.densityOfShooting=1;
scheduleData.periodOfNoShooting=[scheduleData.N-20:scheduleData.N];
...
scheduleData.rewardAvailableInterval = 3* unitaryArray; %randi([1,3],200,1);
...
scheduleData.leftPokeDelay = 0.0001 * unitaryArray;

%Time, in seconds, required in the poke to recieve a reward.
%If set to 0, the reward becomes available as soon as the trial starts.
%When counting pokes, this acts as the maximum inter-poke-interval (minimum poke frequency), defined from poke-in to poke-in.
scheduleData.rightPokeDelay = 0.0001 * unitaryArray;

%Time, in seconds, to hold the left valve open.
%Size of the reward.
scheduleData.leftValveDuration = 0 * unitaryArray;

%Time, in seconds, to hold the right valve open.
%Size of the reward.
%If set to 0, the reward becomes available as soon as the trial starts.
scheduleData.rightValveDuration = 0 * unitaryArray;
...
randDelay = 0.01 * rand(size(unitaryArray));
...
scheduleData.leftRewardDelay = .1*unitaryArray;
...
scheduleData.rightRewardDelay =  .1*unitaryArray;
