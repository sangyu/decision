...
function scheduleData = temporalDiscountingVar15L_2s


scheduleData.LR=0;
scheduleData.bigSize=2.0;
scheduleData.bigDelay=2.0;
scheduleData.smallSize=1;
scheduleData.smallDelay=2;
duration=scheduleData.bigDelay+4;
scheduleData.last2Result=1;
scheduleData.densityOfShooting=1;
scheduleData.periodOfNoShooting=[ 101];

N=101;
unitaryArray=ones(N, 1);
scheduleData.rewardInterval=.2;

scheduleData.bigRewardMatrix=ones(N,scheduleData.bigSize);
scheduleData.smallRewardMatrix=[ones(N,scheduleData.smallSize), zeros(N, scheduleData.bigSize-scheduleData.smallSize)];
scheduleData.timeToShoot='cueon';
% scheduleData.timeToShoot='cuesampled';
scheduleData.timeToShoot=0;


if scheduleData.LR==0
%     scheduleData.timeToShoot='leftdecision';
    scheduleData.leftValveDuration =.04 *scheduleData.bigRewardMatrix;
    ...
    scheduleData.rightValveDuration =.04 *scheduleData.smallRewardMatrix;
...
elseif scheduleData.LR==1
%     scheduleData.timeToShoot='rightdecision';
    scheduleData.rightValveDuration =.04 *  scheduleData.bigRewardMatrix;
    ...
    scheduleData.leftValveDuration =.04 *  scheduleData.smallRewardMatrix;
...
scheduleData.cueSamplingTime = .15 * unitaryArray;
...
scheduleData.leftPokeDelay = .01 * unitaryArray;
...
%If set to 0, the reward becomes available as soon as the trial starts.
%When counting pokes, this acts as the maximum inter-poke-interval (minimum poke frequency), defined from poke-in to poke-in.
scheduleData.rightPokeDelay = .01 * unitaryArray;
