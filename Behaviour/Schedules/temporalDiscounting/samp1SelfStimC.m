...
% * Arbitrary numeric arrays and strings may be included for storage in the header.
function scheduleData = samp1SelfStimC
...
%scheduleData.periodOfNoShooting=[150:190];
...
if scheduleData.LR==0
    scheduleData.leftValveDuration =0 *scheduleData.bigRewardMatrix;
    ...
    scheduleData.rightValveDuration =0 *scheduleData.smallRewardMatrix;
    ...
    scheduleData.rightValveDuration =0 *  scheduleData.bigRewardMatrix;
    ...
    scheduleData.leftValveDuration =0*  scheduleData.smallRewardMatrix;
...
scheduleData.centerPokeDelay = 0.1 * unitaryArray;

%Time, in seconds, required in the center poke to start a valid trial.
scheduleData.cueSamplingTime = .01 * unitaryArray;
...
scheduleData.leftPokeDelay = 0 * unitaryArray;
...
%If set to 0, the reward becomes available as soon as the trial starts.
%When counting pokes, this acts as the maximum inter-poke-interval (minimum poke frequency), defined from poke-in to poke-in.
scheduleData.rightPokeDelay = 0 * unitaryArray;
