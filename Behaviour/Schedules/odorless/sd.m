...
%  .successSounds - The per-trial specification of success tones, each value is interpreted as an index into scheduleData.soundFiles.
%                   Use 0 for no sound for a particular trial.
%  .rewardSounds - The per-trial specification of reward tones, each value is interpreted as an index into scheduleData.soundFiles.
%                  Use 0 for no sound for a particular trial.
...
%               May be upgraded later to represent a programmable interval, after which the pulse will be issued.
%  .leftDecisionTTL - When non-zero, a rising edge is issued on the center poke "valve" line, when a left-side decision is made (poke in), regardless of correctness of the choice.
%                     May be upgraded later to represent a programmable interval, after which the pulse will be issued.
%  .rightDecisionTTL - When non-zero, a rising edge is issued on the center poke "valve" line, when a right-side decision is made (poke in), regardless of correctness of the choice.
%                      May be upgraded later to represent a programmable interval, after which the pulse will be issued.
%  .noResponse - When non-zero, this forces a trial to go to timeout, no responses are considered.
%                The .allowTrialReinitiation field still applies.
%
...
%    fprintf(1, 'Schedule ''sd'' seeding random number generator with %s...\n', num2str(seed));
...
scheduleData.punishmentSounds = ;

%The per-trial duration of punishment tones, in seconds.
scheduleData.punishmentSoundDuration = ;
...
scheduleData.requirePokeForReward = ;
...
scheduleData.cueSampledTTL = ;
%When non-zero, a rising edge is issued on the center poke "valve" line, when a correct decision is made (poke in).
scheduleData.correctDecisionTTL = ;
%When non-zero, a rising edge is issued on the center poke "valve" line, when an incorrect decision is made (poke in).
scheduleData.incorrectDecisionTTL = ;
%When non-zero, a rising edge is issued on the center poke "valve" line, when the reward is released.
scheduleData.rewardTTL = ;
%When non-zero, a rising edge is issued on the center poke "valve" line, when a left-side decision is made (poke in), regardless of correctness of the choice.
scheduleData.leftDecisionTTL = ;
%When non-zero, a rising edge is issued on the center poke "valve" line, when a right-side decision is made (poke in), regardless of correctness of the choice.
scheduleData.rightDecisionTTL = ;

%When non-zero, this forces a trial to go to timeout.
scheduleData.noResponse = ;
