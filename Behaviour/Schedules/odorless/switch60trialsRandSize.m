...
% * Arbitrary numeric arrays and strings may be included for storage in the header.
function scheduleData = switch60trialsRandSize
...
blockLength=30;
numberOfRepeats=8;
singleRewardSize=0.02;
maxNoOfRewards=4;
arrangement=4*ones(numberOfRepeats,2);
for i=1:numberOfRepeats*2
    seed=mod(round(rand*maxNoOfRewards), maxNoOfRewards);
    arrangement(i) = arrangement(i)-seed;
end
unitaryArray = ones(numberOfRepeats*blockLength, 1);

leftRewardSizes=zeros(length(unitaryArray), maxNoOfRewards);
rightRewardSizes=zeros(length(unitaryArray), maxNoOfRewards);

for i=1:numberOfRepeats
    
leftRewardSizes((i-1)*blockLength+1:i*blockLength, :)=[ones(blockLength, arrangement(i,1)), zeros(blockLength, maxNoOfRewards-arrangement(i,1))];
rightRewardSizes((i-1)*blockLength+1:i*blockLength, :)=[ones(blockLength, arrangement(i,2)), zeros(blockLength, maxNoOfRewards-arrangement(i,2))];

end

leftReward=singleRewardSize*leftRewardSizes;
rightReward=singleRewardSize*rightRewardSizes;


scheduleData.rewardNumberRatio=sum(leftRewardSizes, 2)./sum(rightRewardSizes, 2);
scheduleData.leftRewardNumber=sum(leftRewardSizes, 2);
scheduleData.rightRewardNumber=sum(rightRewardSizes, 2);

%% General Setting
%Total time of the trial, from the trial start event (cue onset; centerPokeDelay achieved), until punishment or next trial.
scheduleData.trialDuration = 11 * unitaryArray;

%Time, in seconds, after the trial has started, during which a reward is available.
scheduleData.rewardAvailableInterval = 11 * unitaryArray;
...
%May be a two dimensional array, where the second dimension corresponds to multiple reward deliveries.
scheduleData.leftRewardDelay = 0.5*leftRewardSizes;
%, .5 * unitaryArray
%Time, in seconds, to hold the left valve open.
%Size of the reward.
%May be a two dimensional array, where the second dimension corresponds to multiple reward deliveries.
scheduleData.leftValveDuration = leftReward;
...
%If set to 0, the reward becomes available as soon as the trial starts.
%When counting pokes, this acts as the maximum inter-poke-interval (minimum poke frequency), defined from poke-in to poke-in.
scheduleData.rightPokeDelay = .0001 * unitaryArray;
...
%May be a two dimensional array, where the second dimension corresponds to multiple reward deliveries.
scheduleData.rightRewardDelay = .5 * leftRewardSizes;

%Time, in seconds, to hold the right valve open.
%Size of the reward.
%If set to 0, the reward becomes available as soon as the trial starts.
%May be a two dimensional array, where the second dimension corresponds to multiple reward deliveries.
scheduleData.rightValveDuration = rightReward;
