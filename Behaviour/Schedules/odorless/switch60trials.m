...
% * Arbitrary numeric arrays and strings may be included for storage in the header.
function scheduleData = switch60trials
...
button=rand;
if button>=.5
    bigButton=[ones(60, 1); zeros(60, 1)];
    smallButton=[zeros(60, 1); ones(60, 1)];
    
else
    smallButton=[ones(60, 1); zeros(60, 1)];
    bigButton=[zeros(60, 1); ones(60, 1)];
end

%, 0*unitaryArray
leftReward=[.03 * unitaryArray, .03 * unitaryArray.*bigButton];
rightReward=[.03 * unitaryArray, 0.03 * unitaryArray.*smallButton];
...
%May be a two dimensional array, where the second dimension corresponds to multiple reward deliveries.
scheduleData.leftRewardDelay = [.5 * unitaryArray, .5 * unitaryArray.*bigButton];
%, .5 * unitaryArray
...
%May be a two dimensional array, where the second dimension corresponds to multiple reward deliveries.
scheduleData.rightRewardDelay = [.5 * unitaryArray, .5 * unitaryArray.*smallButton];
