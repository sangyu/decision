...
button=rand;
if button>=.5
    bigButton=[ones(blockLength, 1); zeros(blockLength, 1)];
    smallButton=[zeros(blockLength, 1); ones(blockLength, 1)];
    
else
    smallButton=[ones(blockLength, 1); zeros(blockLength, 1)];
    bigButton=[zeros(blockLength, 1); ones(blockLength, 1)];
end
    bigButton=[bigButton; bigButton];
    smallButton=[smallButton; smallButton];

%, 0*unitaryArray
leftReward=[.025 * unitaryArray, .025 * unitaryArray.*bigButton, .025*unitaryArray.*bigButton];
rightReward=[.025 * unitaryArray, .025 * unitaryArray.*smallButton, .025 * unitaryArray.*smallButton];

scheduleData.leftRewardEnd=leftReward(:, 2);
%% General Setting
%Total time of the trial, from the trial start event (cue onset; centerPokeDelay achieved), until punishment or next trial.
scheduleData.trialDuration = 12 * unitaryArray;
...
%May be a two dimensional array, where the second dimension corresponds to multiple reward deliveries.
scheduleData.leftRewardDelay = [.5 * unitaryArray, .5 * unitaryArray.*bigButton, .5 * unitaryArray.*bigButton];
%, .5 * unitaryArray
...
%May be a two dimensional array, where the second dimension corresponds to multiple reward deliveries.
scheduleData.rightRewardDelay = [.5 * unitaryArray, .5 * unitaryArray.*smallButton, .5 * unitaryArray.*smallButton];
