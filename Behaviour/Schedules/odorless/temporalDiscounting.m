...
% * Arbitrary numeric arrays and strings may be included for storage in the header.
function scheduleData = temporalDiscounting
...
scheduleData.rewardInterval=.05;
scheduleData.smallDelay=0;
duration=10;
unitaryArray=ones(N, 1);
scheduleData.leftReward=ones(N,scheduleData.bigSize);
scheduleData.rightReward=[ones(N,scheduleData.smallSize), zeros(N, scheduleData.bigSize-scheduleData.smallSize)];

%, 0*unitaryArray

scheduleData.leftRewardEnd=scheduleData.leftReward(:, 2);
%% General Setting
...
%May be a two dimensional array, where the second dimension corresponds to multiple reward deliveries.
scheduleData.leftRewardDelay = scheduleData.bigDelay *  scheduleData.rightReward +scheduleData.rewardInterval;

%, .5 * unitaryArray
%Time, in seconds, to hold the left valve open.
%Size of the reward.
%May be a two dimensional array, where the second dimension corresponds to multiple reward deliveries.
scheduleData.leftValveDuration =.03 *  scheduleData.leftReward;
...
%May be a two dimensional array, where the second dimension corresponds to multiple reward deliveries.
scheduleData.rightRewardDelay = scheduleData.smallDelay *  scheduleData.rightReward;

%Time, in seconds, to hold the right valve open.
%Size of the reward.
%If set to 0, the reward becomes available as soon as the trial starts.
%May be a two dimensional array, where the second dimension corresponds to multiple reward deliveries.
scheduleData.rightValveDuration = .03 * scheduleData.rightReward;
