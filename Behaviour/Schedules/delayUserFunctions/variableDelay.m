...
function [scheduleData, nextTrialNumber] = variableDelay(scheduleData, currentTrialNumber, lastResult)
increment=0.25;
%Increment the trial number.
nextTrialNumber = currentTrialNumber + 1;
%Switch the left/right values the animal was correct.
if schduleData.LR==0
    if scheduleData.leftRewardDelay(nextTrialNumber)<20-increment && scheduleData.leftRewardDelay(nextTrialNumber)>increment;
        if lastResult ==1
    ...
    if scheduleData.rightRewardDelay(nextTrialNumber)<20-increment && scheduleData.rightRewardDelay(nextTrialNumber)>increment
        if lastResult ==2
