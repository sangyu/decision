...
% * Arbitrary numeric arrays and strings may be included for storage in the header.
function scheduleData = shapingSamp2
...
scheduleData.maxVarRewardDelay=1;
...
scheduleData.rewardAvailableInterval = 6 * unitaryArray;
...
scheduleData.cueSamplingTime(1:20) = 0.2;
scheduleData.cueSamplingTime(21:scheduleData.N) = 0.3;
...
scheduleData.totalFlow(1:30) = linspace(1, 1000, 30);%Ramp up the air, to get them comfortable with it.
...
randDelay = 0.1 * rand(size(unitaryArray));
...
scheduleData.leftRewardDelay = linspace(1, 3, scheduleData.N);
