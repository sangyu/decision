...
% * Arbitrary numeric arrays and strings may be included for storage in the header.
function scheduleData = shapingSamp1
...
%    fprintf(1, 'Schedule ''Odorsamp2GDRandDelay'' seeding random number generator with %s...\n', num2str(seed));
...
%% easy number settings
scheduleData.N=300;
scheduleData.fraction3rdOdor=0.20;
scheduleData.minRewardDelay=0.2;
scheduleData.maxVarRewardDelay=0.20;
scheduleData.timeToShoot=0;
scheduleData.densityOfShooting=.3;
scheduleData.periodOfNoShooting=[1:40, 150:190];
...
scheduleData.rewardAvailableInterval(1:20) = 10;
scheduleData.rewardAvailableInterval(21:40) = 8;
scheduleData.rewardAvailableInterval(41:scheduleData.N) = 5;
...
scheduleData.cueSamplingTime(1:10) = 0;
scheduleData.cueSamplingTime(11:20) = 0.1;
scheduleData.cueSamplingTime(21:30) = 0.2;
scheduleData.cueSamplingTime(31:scheduleData.N) = 0.3;

%Time, in seconds, that the cue is delivered (unless the animal leaves the poke, at which point delivery is stopped).
scheduleData.cueDuration = 0.5 * unitaryArray;
...
scheduleData.totalFlow(1:30) = 0;
scheduleData.totalFlow(31:scheduleData.N-200) = linspace(1, 1000, length(31:scheduleData.N-200));%Ramp up the air, to get them comfortable with it.
scheduleData.totalFlow(scheduleData.N-199:scheduleData.N) = 1000;%Ramp up the air, to get them comfortable with it.
...
scheduleData.leftPokeDelay = 0.00001 * unitaryArray;
scheduleData.leftPokeDelay(1:15) = 0;%Free delivery.

%Time, in seconds, required in the poke to recieve a reward.
%If set to 0, the reward becomes available as soon as the trial starts.
%When counting pokes, this acts as the maximum inter-poke-interval (minimum poke frequency), defined from poke-in to poke-in.
scheduleData.rightPokeDelay = 0.00001 * unitaryArray;
scheduleData.rightPokeDelay(1:15) = 0;%Free delivery.
...
randDelay = 0.02 * rand(size(unitaryArray));
...
%The per-trial specification of success tones, each value is interpreted as an index into scheduleData.soundFiles.
 scheduleData.successSounds = 0 * unitaryArray;
...
%%%%%%%%%%%%%%%%%%%%%%%%%================= TTLs=====================%%%%%%%%%%%%%%%%%%%%%%%%%%%%
scheduleData.shootr=rand(size(unitaryArray));
scheduleData.shoot=scheduleData.shootr<scheduleData.densityOfShooting;
...
%When non-zero, a rising edge is issued on the center poke "valve" line,
%when the reward is released.
