...
% * Arbitrary numeric arrays and strings may be included for storage in the header.
function scheduleData = threeOdorStimRightDec
% 
...
%A pre-filled array of ones, which may be useful in creating other arrays.
%Of course, this variable may be ignored/removed.

%% easy number settings
scheduleData.N=300;
scheduleData.fraction3rdOdor=0.20;
scheduleData.minRewardDelay=0.2;
scheduleData.maxVarRewardDelay=0.2;
scheduleData.timeToShoot='rightDecision';
...
scheduleData.rewardAvailableInterval = 5 * unitaryArray;
...
scheduleData.cueDuration = 2 * unitaryArray;
...
scheduleData.odor1Valve = double(choiceOfCue <= 0.5*(1-scheduleData.fraction3rdOdor));
scheduleData.odor1Valve(choiceOfCue > 1-scheduleData.fraction3rdOdor) = 2;
...
for i = 1 : length(choiceOfCue)
    if choiceOfCue(i) > 0.5*(1-scheduleData.fraction3rdOdor) && choiceOfCue(i) <= 1-scheduleData.fraction3rdOdor
...
%%%%%%%%%%%%%%%%%%%%%%%%%================= Side Poke and Reward/Punishment=====================%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
scheduleData.left(choiceOfCue > 1-scheduleData.fraction3rdOdor) = 1;
...
scheduleData.right(choiceOfCue > 1-scheduleData.fraction3rdOdor) = 1;

%Randomize the reward delay from 
randDelay=0*unitaryArray;
for i=1:length(unitaryArray)
randDelay(i) = scheduleData.minRewardDelay+scheduleData.maxVarRewardDelay*rand;
end
...
scheduleData.cueLabels = {'Left', 'Right', 'Free'};
