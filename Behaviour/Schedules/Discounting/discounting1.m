...
% * Arbitrary numeric arrays and strings may be included for storage in the header.
function scheduleData = ThreeOdorRandDelay33long
...
scheduleData.odor1Valve = double(choiceOfCue <= 0.33);
scheduleData.odor1Valve(choiceOfCue > 0.66) = 2;
...
for i = 1 : length(choiceOfCue)
    if choiceOfCue(i) > 0.33 && choiceOfCue(i) <= 0.66
...
unitRewardScaling = [small; big; 1; 1; small; big; 1; 1]
unitReverseRewardScaling = 1./unitRewardScaling
unitTimeScaling = [1; 1; big; small; 1; 1; big; small]
unitReverseTimeScaling = 1./unitTimeScaling

rewardScaling =[];
reverseRewardScaling = [];
timeScaling= [];
reverseTimeScaling = [];

for i=1:length(unitRewardScaling)
	rewardScaling(repeat*(i-1)+1:repeat*(i),:)= unitRewardScaling(i);
	reverseRewardScaling(repeat*(i-1)+1:repeat*(i),:)= unitReverseRewardScaling(i);
	timeScaling(repeat*(i-1)+1:repeat*(i),:)= unitTimeScaling(i);
	reverseTimeScaling(repeat*(i-1)+1:repeat*(i),:)= unitReverseTimeScaling(i);
end
...
%Size of the reward.
scheduleData.leftValveDuration = 0.04 * rewardScaling;

%Time, in seconds, to hold the right valve open.
%Size of the reward.
%If set to 0, the reward becomes available as soon as the trial starts.
scheduleData.rightValveDuration = 0.04 * reverseRewardScaling;
...
scheduleData.left(choiceOfCue > 0.66) = 1;
...
scheduleData.right(choiceOfCue > 0.66) = 1;
...
scheduleData.leftRewardDelay = 0.5 * timeScaling;
...
scheduleData.rightRewardDelay = 0.5 * reverseTimeScaling;
