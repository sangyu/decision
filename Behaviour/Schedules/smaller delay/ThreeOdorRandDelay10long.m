...
% * Arbitrary numeric arrays and strings may be included for storage in the header.
function scheduleData = ThreeOdorRandDelay10long
...
scheduleData.odor1Valve = double(choiceOfCue <= 0.45);
scheduleData.odor1Valve(choiceOfCue > 0.90) = 2;
...
for i = 1 : length(choiceOfCue)
    if choiceOfCue(i) > 0.45 && choiceOfCue(i) <= 0.90
...
scheduleData.left(choiceOfCue > 0.90) = 1;
...
scheduleData.right(choiceOfCue > 0.90) = 1;
