...
function [scheduleData, nextTrialNumber] = variableDelay15double(scheduleData, currentTrialNumber, lastResult)

%Increment the trial number.
nextTrialNumber = currentTrialNumber + 1;
%Switch the left/right values the animal was correct.
if scheduleData.LR==0
    if lastResult ==1 && scheduleData.last2Result==1
       if scheduleData.leftRewardDelay(nextTrialNumber)<15
            scheduleData.leftRewardDelay(nextTrialNumber:end, 1)= scheduleData.leftRewardDelay(currentTrialNumber, 1)+ 0.5;
       end
    elseif lastResult==2   && scheduleData.last2Result==2    
       if scheduleData.leftRewardDelay(nextTrialNumber)>.5
            scheduleData.leftRewardDelay(nextTrialNumber:end, 1)= scheduleData.leftRewardDelay(currentTrialNumber, 1)- 0.5;
       end 
    end
else
   if lastResult ==2&& scheduleData.last2Result==2
       if scheduleData.rightRewardDelay(nextTrialNumber)<15
            scheduleData.rightRewardDelay(nextTrialNumber:end, 1)= scheduleData.rightRewardDelay(currentTrialNumber, 1)+ 0.5;
       end
    elseif lastResult==1   && scheduleData.last2Result==1    
       if scheduleData.rightRewardDelay(nextTrialNumber)>0.5
            scheduleData.rightRewardDelay(nextTrialNumber:end, 1)= scheduleData.rightRewardDelay(currentTrialNumber, 1)- 0.5;
       end 
   end    
end

scheduleData.last2Result=lastResult;
